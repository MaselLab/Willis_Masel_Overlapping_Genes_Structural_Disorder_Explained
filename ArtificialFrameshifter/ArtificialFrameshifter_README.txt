...
The purpose of this program is to take in data from a tab-delimited file, and return two different nucleotide sequences in tab-delimited format. The two sequences will be the original sequences artificially frameshifted into the +1 and +2 frames. The results will be printed to two tab-delimited files. For a somewhat cleaner workspace, the output files will be saved one directory up from where the python script is stored.
...
The main program is stored in the directory ArtificialFrameshiftProgram, and is titled FrameshiftFromFile_TabDelimited.py

All other .py programs stored in the listed directory act as accessories to the main file. 

The non-overlapping genes that were used for this project are stored in this directory as well in the flat file: VirusGenes_NonOverlappingControls_Complete.txt
...
5) Removes any stop codons that may have arisen in the body of the sequence as a result of the frameshift
